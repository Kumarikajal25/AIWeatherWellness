...
import {  ImgPlusIcon, MicIcon, SendIcon,StopIcon, VercelIcon } from '@/app/utils/Icons';
...
function Main() {
  ...
  return (
    <div className={clsx('flex flex-row  h-auto     w-auto', Icon ? 'bg-black/50   min-w-fit ':'bg-inherit'  )}>
        <div className='sidebarComponet  min-h-screen'>
            <Sidebar/>
        </div>
        <div className='mainComponent w-full  animate-fadeIn '>
        <nav className=' flex flex-row  justify-between  p-4 items-center '>
        <p  className=' text-2xl  text-zinc-600 dark:text-zinc-400 font-sans'>Gemini</p>
      ...
      <div className='  w-auto   h-screen  lg:h-auto' >
        <div className= {clsx('   h-full  mx-4  flex  justify-between  flex-col   gap-9 xl:mt-12',Icon ? ' xl:mx-48 lg:mx-32 ' :'  lg:mx-12 xl:mx-28')}>
            {!showResult ?<>
                <div className=' Tittle flex  flex-col   gap-1  text-4xl sm:text-5xl  text-neutral-400 '>
           ...
           <div className= {clsx(Icon ? '-z-10' :' z-0 , grid  md:grid-cols-2  grid-cols-1  lg:grid-cols-3 xl:grid-cols-4  gap-4')}>
        
              {
               ...
               <div className=' bg-white rounded-full p-3 relative dark:bg-neutral-900'>
                   ...
                   <div className=' Result     scroll-bar overflow-y-scroll'>
         ...
         <p className=' text-lg  font-sans' dangerouslySetInnerHTML={{ __html: resultData }}></p>
                     ...
                     {/* Bottom-part */}
                     <div className= { clsx(showResult?' absolute bottom-2  mr-3 sm:mr-0  min-w-fit  w-9/12':'flex  flex-col   w-full   items-center gap-3 lg:gap-6',Icon ? '-z-10' :' z-0')}>
                     <div className= {clsx('bottom-box  min-w-fit w-full bg-cardColor  dark:bg-blackbg  rounded-3xl',showResult?' relative bottom-3' :' bottom-0')}>
                        <div className='input-box flex flex-row justify-between items-center p-5'>
                        <input  onChange={(e)=>setInput(e.target.value)}  onKeyDown={handleKeyDown}
                         value={input} type='text'  className=' flex-1 border-none outline-none bg-transparent text-lg px-5'placeholder='Enter a prompt here ...'></input>
